...
# Tennis Interview Clip 002 - UI-Based Real Transcription
# Uses UI approach instead of service to avoid "process is bad" errors

set -e

echo "🎾 Tennis Interview Clip 002 - UI-Based Real Transcription"
echo "========================================================="
echo "Device: Xiaomi Pad Ultra"
echo "Goal: Use UI approach to avoid service process errors and get real transcription"
echo "Timestamp: $(date)"
echo ""
...
# Results directory
RESULTS_DIR="./tennis_clip_ui_based_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$RESULTS_DIR"
...
# Step 5: UI-Based Processing
echo "=== STEP 5: UI-Based Processing ==="
echo "🚀 Starting UI-BASED transcription..."
START_TIME=$(date +%s)

echo "📱 Launching app with UI-based processing..."
...
echo "✅ UI-BASED processing launched successfully"
echo ""

# Step 6: UI-Based Monitoring
echo "=== STEP 6: UI-Based Monitoring ==="
PROCESSING_COMPLETE=false
WAIT_TIME=0
MAX_WAIT_TIME=180  # 3 minutes for UI-based processing
...
while [ $WAIT_TIME -lt $MAX_WAIT_TIME ] && [ "$PROCESSING_COMPLETE" = "false" ]; do
    ...
    echo "🔍 UI-Based Monitoring Step $STEP_COUNT (${WAIT_TIME}s elapsed)..."
    ...
    RECENT_LOGS=$(adb logcat -d | grep -E "(TranscribeWorker|whisper|TECHNICAL|WhisperMainActivity|WhisperJNI)" | tail -8)
...
if [ "$TRANSCRIPT_FOUND" = "true" ]; then
    echo "🎉 SUCCESS: Real transcription completed with UI-BASED processing!"
    ...
    echo ""
    echo "📋 Recent processing logs:"
    adb logcat -d | grep -E "(TranscribeWorker|whisper|TECHNICAL|WhisperMainActivity|WhisperJNI)" | tail -25
fi
...
adb logcat -d | grep -E "(TranscribeWorker|whisper|TECHNICAL|WhisperMainActivity|WhisperJNI)" > "$RESULTS_DIR/whisper_logs.txt"
...
- UI-based processing instead of service (avoids "process is bad" error)
- Reduced threads to 2 to avoid resource conflicts
- Forced English language processing
- Extended monitoring for UI-based processing
...
if [ "$TRANSCRIPT_FOUND" = "true" ]; then
    ...
    echo "🎉 SUCCESS: UI-based processing completed with real transcription!"
else
...
echo ""
echo "🎾 Tennis interview UI-based test completed!"
